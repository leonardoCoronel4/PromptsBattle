  ...
  try {
    ...
    socket.on("joinMatch", async function (data, matchData) {
      ...
      if (
        ...
        const winnerSpectateResponse = await fetch(`/partials/winner.html`);
        ...
        const playerWinnerImage = document.getElementById("player-image");
        playerWinnerImage.src = match.imagenWinner;
        const topic = document.getElementById("topic");
        topic.textContent = match.topic;
        const quitMatchButton = document.getElementById("quitMatchButton");
        ...
        socket.on("currentText", (matchData, matchId) => {
          ...
          if (matchData && matchData[match.playerOneSession + matchId]) {
            playerOneText.textContent =
              matchData[match.playerOneSession + matchId].playerText || "";
          }
          if (matchData && matchData[match.playerTwoSession + matchId]) {
            playerTwoText.textContent =
              matchData[match.playerTwoSession + matchId].playerText || "";
          }
        });
        ...
        socket.on("currentImages", (matchData, matchId) => {
          ...
          if (matchData && matchData[match.playerOneSession + matchId]) {
            playerOneImageOne.src =
              matchData[match.playerOneSession + matchId].images[1] ||
              "../images/incognita.png";

            playerOneImageTwo.src =
              matchData[match.playerOneSession + matchId].images[2] ||
              "../images/incognita.png";

            playerOneImageThree.src =
              matchData[match.playerOneSession + matchId].images[3] ||
              "../images/incognita.png";
          }

          if (matchData && matchData[match.playerTwoSession + matchId]) {
            playerTwoImageOne.src =
              matchData[match.playerTwoSession + matchId].images[1] ||
              "../images/incognita.png";

            playerTwoImageTwo.src =
              matchData[match.playerTwoSession + matchId].images[2] ||
              "../images/incognita.png";

            playerTwoImageThree.src =
              matchData[match.playerTwoSession + matchId].images[3] ||
              "../images/incognita.png";
          }
        });
        socket.on(
        ...
        socket.on(
          ...
          (imgUrl) => {
            const playerText = document.getElementById("player-two-screen");
            playerText.remove();
            const playerContenido =
              document.getElementById("player-two-images");
            playerContenido.remove();
            const image = document.createElement("img");
            const container = document.getElementById("playerTwoContainer");
            image.src = imgUrl;
        ...
        socket.on(
          ...
          (imgUrl) => {
            const playerText = document.getElementById("player-one-screen");
            playerText.remove();
            const playerContenido =
              document.getElementById("player-one-images");
            playerContenido.remove();
            const image = document.createElement("img");
            const container = document.getElementById("playerOneContainer");
            image.src = imgUrl;
            image.classList.add("imagenSelect");
            container.appendChild(image);
        ...
        socket.on("currentImagenSeleccionada", (matchData, matchId) => {
          if (
            ...
            const image = document.createElement("img");
            const container = document.getElementById("playerOneContainer");
            image.src = matchData[match.playerOneSession + matchId].imagenFinal;
            image.classList.add("imagenSelect");
            container.appendChild(image);
            playerOneFinished = true;
          }
          ...
          if (
            ...
            const image = document.createElement("img");
            const container = document.getElementById("playerTwoContainer");
            image.src = matchData[match.playerTwoSession + matchId].imagenFinal;
            image.classList.add("imagenSelect");
            container.appendChild(image);
            playerTwoFinished = true;
          }
        ...
        socket.on("updateVotesButton", (matchRecibido) => {
          ...
          if (votarButton1 && votarButton2) {
            ...
            let totalVotes =
              matchRecibido.playerOneVotes + matchRecibido.playerTwoVotes;
            if (totalVotes > 0) {
